...
### 4. FHEVM host-contracts stub (cca root build)

- **File:** `lib/fhevm/host-contracts/addresses/FHEVMHostAddresses.sol` (created)
- Stub with Sepolia fhEVM addresses so `forge build` at **cca** root succeeds (that file is normally generated by fhevm deploy tasks).
...
- `lib/fhevm/host-contracts/addresses/FHEVMHostAddresses.sol` — stub for build.
